...
def run_unified_benchmark():
    ...
    if os.path.exists(RESULTS_FILE):
        ...
        all_results = []
    print(f"Resuming Unified Entropy Benchmark (Greedy NST/NSF)...")
    print(f"Skipping completed datasets (interpolated CIFAR-10).")
    
    # Filter datasets to only run SHD and DVSGesture
    target_datasets = ['SHD', 'DVSGesture']
    available_datasets = {k: v for k, v in datasets.items() if k in target_datasets}

    for d_name, d_loader in available_datasets.items():
        ...
        if d_name in ['MNIST', 'Fashion-MNIST', 'UCI HAR', 'SHD', 'DVSGesture'] and X.shape[1] > 50:
        ...
        for train_idx, test_idx in skf.split(X, y):
            ...
            for lam in LAMBDAS:
                ...
                # Unpack 3 values: preds, lats, sops
                preds, lats, s_counts = nst.predict_with_latency(X_test_spikes)
                acc = accuracy_score(y_test, preds)
                lat = np.mean(lats)
                sop = np.mean(s_counts)
                depth = nst.get_depth()
                ...
                print(f"    [NST] lam={lam}: Acc={acc:.4f}, Lat={lat:.1f}, SOPs={sop:.0f}")
                ...
                # NSF predict now returns (final_preds, final_lats, final_sops)
                preds, lats, s_counts = nsf.predict(X_test_spikes)
                acc = accuracy_score(y_test, preds)
                lat = np.mean(lats)
                sop = np.mean(s_counts)
                ...
                print(f"    [NSF] lam={lam}: Acc={acc:.4f}, Lat={lat:.1f}, SOPs={sop:.0f}")
